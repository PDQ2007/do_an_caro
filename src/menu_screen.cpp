...
namespace strings{
	std::wstring
		title[2] = {
			L"This is a menu screen",
			L"Đây là Màn hình menu"
		},
		open_win2[2] = {
			L"Click here to open Play screen",
			L"Bấm vào đây để mở Màn hình chuẩn bị chơi"
		},
		open_win3[2] = {
...
namespace events{
	void Win2(sf::RenderWindow& win){
		if(win2.getGlobalBounds().contains(win.mapPixelToCoords(sf::Mouse::getPosition(win)))){
...
void initMenuScreen(sf::RenderWindow& win){
	title.setString(strings::title[globalConfig::language]);
	win2.setString(strings::open_win2[globalConfig::language]);
...
void loopMenuScreen(sf::RenderWindow& win){
	
	bool is_loop = true;

	while(is_loop){

		if(globalConfig::current_win != 1 || globalConfig::re_init) return;
...
void drawMenuScreen(sf::RenderWindow& win){
	initMenuScreen(win);
	//std::cout << "check";
	loopMenuScreen(win);
};
